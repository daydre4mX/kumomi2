...
## Usage instructions
One must modify the ```config.properties.example``` within ```/src``` in order to match the settings corresponding to the user's MySQL database. The program assumes the port of the database is 3306.
```
...
You must also rename ```config.properties.example``` to ```config.properties```
